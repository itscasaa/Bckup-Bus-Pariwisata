...
const NewsCard = ({ item, onEdit, onDelete, deleting }) => {
  ...
  const imgSrc = item.gambar
    ? (item.gambar.startsWith('http') ? item.gambar : `${API}/frontend/assets/images/${item.gambar}`)
    : null;
        ...
        <section>
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-unit-lg">
            ...
            {!loading && filtered.length === 0 && (
              <EmptyState onAdd={() => navigate('/news/tambah')} />
            )}
            ...
            {/* Add new card — selalu tampil di akhir jika ada data */}
            {!loading && filtered.length > 0 && (
              <AddCard onClick={() => navigate('/news/tambah')} />
            )}
          </div>
        </section>
...
export function TambahNews() {
  ...
  const handleSubmit = async (form) => {
    ...
    try {
      const res  = await fetch(`${API}/admin/api/tambah_news.php`, {
...
export function EditNews() {
  ...
  const handleSubmit = async (form) => {
    setError(''); setLoading(true);
    try {
      const res  = await fetch(`${API}/admin/api/edit_news.php?id=${id}`, {
        method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(form),
      });
      const data = await res.json();
      if (data.status === 'success') navigate('/news');
